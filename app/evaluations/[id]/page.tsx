...
export default function ViewEvaluationPage() {
  ...
  const loadAllData = async () => {
    try {
      ...
      if (canViewSummaryAverage(viewer.preferred_role, ev)) {
        try {
          ev.summary_group_average = (
            await fetchGroupAveragePool(ev.id!)
          ).average;
        } catch (e) {
        ...
        if (ev.summary_group_id) {
          try {
            ev.summary_group_distribution = (
              await fetchGroupDistribution(ev.id!)
            ).distribution;
          } catch (e) {
  ...
  if (error || !evaluation || !profile)
    return (
      <LoadError message={error} onBack={() => router.push("/dashboard")} />
    );

  const canEdit = canPerformAction(profile, "edit_evaluation", evaluation);
  const canSeeSummaryAvg = canViewSummaryAverage(
    profile.preferred_role,
    evaluation,
  );
  const onSign: OnSign = (block, label, signer) =>
    setSigning({ block, label, signer });
  ...
  return (
    ...
    >
      <div className="space-y-6">
        <ReportBanner
          evaluation={evaluation}
          showSummaryAverage={canSeeSummaryAvg}
        />
        ...
        {activeTab === "review" && (
          <div className="space-y-4">
            <p className="apex-info-box">
              ...
              — route this report through the chain of command, attach summary
              groups, recycle for correction, or begin debrief.
            </p>
            <ReviewPanel
              evaluation={evaluation}
              currentUser={profile}
        ...
        {signing && (
          <CredentialSignatureModal
            ...
            defaultTypedName={
              signing.block === 51 || signing.block === 32
