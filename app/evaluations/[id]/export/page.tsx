...
export default function EvaluationExportPage() {
  ...
  const [forcedDist, setForcedDist] = useState<ForcedDistributionResult | null>(
    null,
  );
    ...
    const loadData = async () => {
      try {
        ...
        if (canViewSummaryAverage(viewer.preferred_role, data)) {
          try {
            data.summary_group_average = (
              await fetchGroupAveragePool(data.id!)
            ).average;
          } catch (e) {
          ...
          if (data.summary_group_id) {
            try {
              ...
              setForcedDist(
                checkForcedDistribution(distribution, data.grade_rate),
              );
            } catch (e) {
          ...
          {success ? (
            <div className="p-4 rounded-xl border border-emerald-500/20 bg-emerald-950/20 space-y-2">
              ...
              <p className="text-xs text-emerald-300/80 leading-relaxed">
                All BUPERSINST 1610.10H constraints, including administrative
                rules, trait rating promotion gates, and the Block 43 Courier
                text bounds, have passed successfully.
              </p>
            </div>
          ) : (
            <div className="p-4 rounded-xl border border-red-500/20 bg-red-950/20 space-y-2">
              ...
              <p className="text-xs text-red-300/80 leading-relaxed">
                The evaluation has failed the pre-export quality checks. You
                must resolve these issues before you can download or finalize
              ...
              <div className="space-y-2">
                ...
                <ul className="space-y-2">
                  {errors.map((err, idx) => (
                    <li
                      key={idx}
                      className="p-3 bg-red-950/10 border border-red-900/20 rounded-lg text-xs text-red-200 flex items-start gap-2.5"
                    >
            ...
            {warnings.length > 0 && (
              <div className="space-y-2">
                <h4 className="text-xs font-bold text-amber-400 uppercase tracking-wider">
                  Policy Advisories / Warnings
                </h4>
                <ul className="space-y-2">
                  {warnings.map((warn, idx) => (
                    <li
                      key={idx}
                      className="p-3 bg-amber-950/10 border border-amber-900/20 rounded-lg text-xs text-amber-200 flex items-start gap-2.5"
                    >
              ...
              <ul className="space-y-2">
                {forcedDist.violations.map((v, i) => (
                  ...
                  >
                    <span className="text-red-400 font-bold mt-0.5">✕</span>
                    <span>
                      {v.message} This report cannot be finalized until the
                      Reporting Senior brings the group within limits.
                    </span>
                  </li>
                ))}
              </ul>
        ...
        <div id="apex-pdf-preview-section" className="space-y-3">
          ...
          {!success && (
            <p className="text-xs text-amber-300/80 leading-relaxed">
              Live preview of the report as drafted, so you can track progress.
              It still has open validation blockers and cannot be downloaded or
              finalized until they are resolved.
            </p>
          )}
            ...
            {!success ? (
              ...
              <button
                ...
                disabled={
                  isFinalizing ||
                  evaluation.status === "completed" ||
                  (forcedDist != null && !forcedDist.compliant)
                }
                title={
                  forcedDist != null && !forcedDist.compliant
