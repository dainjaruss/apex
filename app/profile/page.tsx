...
const getErrorMessage = (err: any) => err?.message || 'Failed to update profile settings'
...
function valToRole(v: any): 'Sailor' | 'Rater' | 'Senior Rater' | 'Reporting Senior' | 'Admin' {
  if (v === 'Sailor' || v === 'Rater' || v === 'Senior Rater' || v === 'Reporting Senior' || v === 'Admin') {
    return v
  }
  return 'Sailor'
}
  ...
  return (
    <header className="px-6 py-4 flex items-center justify-between border-b border-[#1c2541] glass-panel">
      <div className="flex items-center gap-3">
        <Link href="/dashboard" className="font-extrabold text-xl tracking-wider text-white hover:text-[#91aec9] transition-colors">
          APEX
        </Link>
        <span className="text-xs px-2.5 py-0.5 rounded-full bg-[#1c2541] text-[#3e6e99]">PROFILE</span>
      </div>
      <Link href="/dashboard" className="text-xs font-semibold text-[#91aec9] hover:text-white transition-colors">
        Back to Dashboard
      </Link>
    </header>
  )
...
function ProfileInput({
  ...
  return (
    <div className={`space-y-1.5 ${className}`}>
      <label className="text-xs font-semibold text-[#91aec9] uppercase tracking-wider">{label}</label>
      <input
        type={type}
        required={required}
        maxLength={maxLength}
    ...
    <>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <ProfileInput
          ...
          onChange={(e) => onChange('dodId', e.target.value.replace(/[^0-9]/g, ''))}
  ...
  return (
    <>
      ...
      <div className="space-y-1.5">
        <label className="text-xs font-semibold text-[#91aec9] uppercase tracking-wider">Preferred Evaluation Role</label>
        <select
          value={formData.role}
        ...
        >
          ...
          <option value="Senior Rater">Senior Rater (Division Officer / Chief)</option>
          <option value="Reporting Senior">Reporting Senior (Commanding Officer)</option>
...
function ProfileFormFields({ formData, loading, onChange, onSubmit }: ProfileFormFieldsProps) {
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <ProfilePersonalFields formData={formData} onChange={onChange} />
      
      <div className="space-y-1.5">
        <label className="text-xs font-semibold text-[#608bb3] uppercase tracking-wider">Email (Read Only)</label>
        <input
          type="email"
          disabled
    ...
    <main className="flex-1 max-w-xl w-full mx-auto p-6 flex items-center justify-center">
      <div className="w-full p-8 rounded-2xl glass-panel space-y-6">
        <div className="text-center space-y-2">
          <h2 className="text-2xl font-bold text-white tracking-wide">Military Identity</h2>
          <p className="text-sm text-[#91aec9]">Update rank, command, and routing role permissions</p>
        </div>
