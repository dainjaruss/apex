...
function FormInput({ label, error, fieldClass, containerClassName = "space-y-1.5", ...props }: FormInputProps) {
  return (
...
function FormSelect({ label, error, fieldClass, containerClassName = "space-y-1.5", children, ...props }: FormSelectProps) {
  return (
    <div className={containerClassName}>
      <label className="text-xs font-semibold text-[#91aec9] uppercase tracking-wider">{label}</label>
  ...
  return (
    <div className="flex min-h-screen bg-[#0b132b] items-center justify-center p-4">
      <div className="w-full max-w-lg p-8 rounded-2xl glass-panel space-y-6 text-center">
        <h2 className="text-2xl font-bold text-green-400 tracking-wide">Registration Submitted</h2>
        <p className="text-[#91aec9] text-sm">
          We have sent a secure verification link to <strong>{email}</strong>. 
          Please check your inbox and click the link to confirm your identity before logging in.
        </p>
        <div className="pt-6">
          <Link href="/login" className="text-sm font-semibold text-[#3e6e99] hover:text-[#91aec9] transition-colors">
            Return to Login
          </Link>
        </div>
      </div>
...
function PersonalFields({ formData, onChange, fieldErrors, fieldClass }: SectionProps) {
  return (
    <>
      <FormInput
        id="reg-first-name"
      ...
      <FormInput
        ...
        onChange={(e) => onChange('middleInitial', e.target.value.toUpperCase())}
...
function ProfessionalFields({ formData, onChange, fieldErrors, fieldClass }: SectionProps) {
  return (
    <>
      <FormSelect
        id="reg-rank"
      ...
      <FormInput
        ...
        label={
          <>
            UIC <span className="text-[#608bb3] normal-case font-normal">(optional)</span>
          </>
        }
        type="text"
        maxLength={5}
...
function AccountFields({ formData, onChange, fieldErrors, fieldClass }: SectionProps) {
  return (
    <>
      <FormInput
        id="reg-email"
...
function useRegisterForm() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = error instanceof Error ? error.message : 'Registration failed. Please review inputs.'
      setServerError(msg)
    } finally {
      setLoading(false)
    }
