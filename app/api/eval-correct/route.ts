...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!evaluationId || !patch || typeof patch !== "object")
    ...
    if (ev.routing_stage !== "debrief")
      return fail("Corrections are only open during debrief.", 409);
    if (!(ev.participants || []).includes(callerId))
      return fail(
    ...
    await admin.from("audit_logs").insert([
      {
        ...
        user_id: callerId,
        action: "MINOR_CORRECTION_APPLIED",
        details: {
          keys: Object.keys(patch).filter((k) =>
            MINOR_CORRECTION_KEYS.includes(k),
          ),
        },
      },
    ]);
