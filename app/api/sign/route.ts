...
import {
  parseSignRequest, verifyCredentials, loadSignContext, authorizeSigner, applySignature, isSignFailure,
} from '@/lib/signing'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { evaluationId, blockNum, key, email, password, typedName, signatureDataUrl } = parsed.data
    ...
    const result = await applySignature(
      admin, ctx.evaluation, blockNum, key, typedName, signatureDataUrl, verified.signerId, ctx.signer.preferred_role
    )
    if (isSignFailure(result)) return fail(result.error, result.status)

    return NextResponse.json({ ok: true, block_values: result.block_values }, { status: 200 })
  } catch (error: any) {
