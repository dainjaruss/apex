...
export const STARRED_BILLET_SUBCATEGORIES = [
  'CRF', 'CANVASSER', 'RESIDENT', 'INTERN', 'STUDENT',
] as const;
...
export const EvalSchema = z.object({
  member_name: z
    ...
    .regex(/^[A-Za-z\s-]+,\s*[A-Za-z\s-]+$/, 'Name must be in LAST, FIRST MI format and contain only letters, spaces, hyphens, and a single comma (Block 1)'),

  grade_rate: z
    .string()
    .min(1, 'Grade/Rate is required (Block 2)')
    .regex(/^[a-zA-Z0-9]+$/, 'Grade/Rate must contain letters and numbers only (Block 2)'),
  ...
  date_reported: z
    .string()
    .regex(/^\d{4}-\d{2}-\d{2}$/, 'Date Reported must be a valid date in YYYY-MM-DD format (Block 9)'),
  ...
  physical_readiness: z
    ...
    .regex(/^[PBFMWN]+$/, 'Physical readiness (Block 20) must use only codes P, B, F, M, W, N — no spaces, slashes, or other characters.'),

  billet_subcategory: z
    .string()
    .min(1, 'Billet Subcategory is required — select NA if not used (Block 21)')
    ...
    .refine((v) => !v || BILLET_SUBCATEGORY_OPTIONS.includes(v), {
      message: 'Billet Subcategory must be a valid code from BUPERSINST 1610.10H table 1-1 — e.g., NA, BASIC, CRF, or SPECIAL01–SPECIAL20 (Block 21)',
    }),
  ...
  reporting_senior_grade: z
    ...
    .regex(/^[A-Za-z0-9]+$/, 'Reporting Senior grade must contain letters/numbers only, no hyphens (Block 23)'),
  ...
  reporting_senior_title: z
    ...
    .max(14, 'Reporting Senior title must be 14 characters or fewer (Block 25)'),
  ...
  reporting_senior_dod_id: z
    .string()
    .regex(/^[0-9]{10}$/, 'Reporting Senior DoD ID must be exactly 10 digits (Block 27)'),
  ...
  primary_duty_abbrev: z
    .string()
    .max(PRIMARY_DUTY_ABBREV_MAX, `Primary duty abbreviation must be ${PRIMARY_DUTY_ABBREV_MAX} characters or fewer, including spaces (Block 29A)`)
    .regex(/^[A-Za-z0-9 /-]*$/, 'Primary duty abbreviation may contain only letters, numbers, spaces, slashes, and hyphens (Block 29A)')
    .optional(),
  ...
  counselor: z
    ...
    .max(COUNSELOR_MAX, `Counselor must be ${COUNSELOR_MAX} characters or fewer to fit the form (Block 31)`),
  ...
  career_recommendations: z
    ...
    .max(CAREER_REC_SLOTS, `Block 41 allows at most ${CAREER_REC_SLOTS} career recommendations`)
    .refine((arr) => arr.some((r) => r.trim().length > 0), {
      message: 'At least one career recommendation is required — do not leave blank; enter NA or NONE if none applies (Block 41)',
    }),
  ...
  if (/^\d{4}-\d{2}-\d{2}$/.test(data.date_reported)) {
    ...
    const isRealDate =
      reported.getFullYear() === y && reported.getMonth() === m - 1 && reported.getDate() === d;
    const now = new Date();
  ...
  if (desig && !/^([0-9]{4}|LTR|USAF|USA|USMC|USCG|USSF|USPH|NOAA)$/.test(desig)) {
    ctx.addIssue({
      path: ['reporting_senior_designator'],
      code: z.ZodIssueCode.custom,
      message: 'Reporting Senior designator (Block 24) must be a 4-digit officer designator, "LTR", or a Service abbreviation (USAF, USA, USMC, USCG, USSF, USPH, NOAA) — or left blank.',
    });
  }
  ...
  if (dcUpper !== 'NOT REQ' && dcUpper !== 'NOT PERF' && !dcIsDate) {
    ctx.addIssue({
      path: ['date_counseled'],
      code: z.ZodIssueCode.custom,
      message: 'Date Counseled must be a valid date, NOT REQ, or NOT PERF (Block 30)',
    });
  }
