...
export default function TraitRow({ traitKey, label, value, error, onChange, gradeValues, onFocus }: TraitRowProps) {
    ...
    >
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-2">
        <div className="mb-2 md:mb-0">
          ...
          {std?.definition && <p className="text-xs text-slate-500 mt-0.5">{std.definition}</p>}
      ...
      {value && <TraitStandardPanel traitKey={traitKey as TraitKey} grade={value} />}
    </div>
  )
}
