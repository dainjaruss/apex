...
export default function CredentialSignatureModal({
  ...
  const handleSubmit = async () => {
    setError(null);
    if (!typedName.trim()) {
      setError("Enter your typed name.");
      return;
    }
    if (!email.trim() || !password) {
      setError("Enter your username (email) and password to sign.");
      return;
    }
    if (!consent) {
      setError("You must certify the signature to proceed.");
      return;
    }

    setSubmitting(true);
    try {
      const signatureDataUrl = hasDrawn
