...
import {
  routeForward,
  recycleForCorrection,
  beginDebrief,
  applyMinorCorrection,
  setLock,
...
export default function ReviewPanel({
  ...
  useEffect(() => {
    if (evaluation.id)
      fetchReviewApprovals(evaluation.id)
  ...
  const canDebrief = hasPermission(
    ...
    <div className="apex-card p-6 space-y-6">
      <div className="border-b pb-3" style={{ borderColor: "var(--border)" }}>
        ...
        >
          ...
          {locked && (
            <span className="ml-2 text-amber-400 font-semibold">· Locked</span>
          )}
        </p>
      </div>
      ...
      {locked ? (
        <LockedBanner
          ...
          loading={loading}
          onUnlock={() => run(() => setLock(evaluation.id!, false))}
        />
      ) : stage === "debrief" ? (
        <DebriefActions
          evaluation={evaluation}
        ...
        <p
          className="text-sm rounded-lg p-4 apex-card"
          style={{ color: "var(--muted-foreground)" }}
        >
          This report is currently with the{" "}
          <span className="font-semibold text-white">
            {STAGE_LABEL[stage] || stage}
          </span>
          . You'll regain access if it is recycled to you or opened for debrief
          corrections.
        </p>
      )}
...
function HolderActions({
  ...
  return (
    <div className="space-y-5">
      ...
      />
      {stage !== "sailor" && (
        <RecycleAction evaluation={evaluation} run={run} loading={loading} />
      )}
...
function RouteForward({
  ...
  return (
    <div className="space-y-2">
      {stage === "sailor" && (
        <GroupPicker evaluation={evaluation} run={run} loading={loading} />
      )}
...
function GroupPicker({
  ...
  return (
    <div
      className="space-y-2 pb-3 mb-1 border-b"
    ...
    >
      ...
      >
        Only groups matching your paygrade, promotion status, ending date, and
        reporting senior are shown
        {memberPaygrade
          ? ` (${evaluation.grade_rate} · ${memberPaygrade})`
          : ""}
        .
      </p>
      {visible.length === 0 ? (
        <p className="text-xs text-slate-500 italic">
          No eligible open summary groups for this report.
        </p>
      ) : (
        <select
          className={FIELD}
      ...
      <button
        ...
        disabled={loading || (sel !== "" && !visible.some((g) => g.id === sel))}
        onClick={() =>
          run(() => attachSummaryGroup(evaluation.id!, sel || null))
        }
      >
        Attach Group
      </button>
  ...
  return (
    ...
    >
      <label className="apex-label">
        Recycle for correction (one step back)
      </label>
      <textarea
      ...
      <button
        ...
        onClick={() =>
          run(() => recycleForCorrection(evaluation.id!, comments))
        }
      >
...
function DebriefActions({
  ...
  const isParticipant = (evaluation.participants || []).includes(
    currentUser.id,
  );
  return (
    <div className="space-y-4">
      <p className="text-sm text-amber-300 bg-amber-950/20 border border-amber-900/40 rounded-lg p-3">
        Debrief in progress — participants may make minor corrections. The
        Reporting Senior signs Block 50 (Details tab) to lock the report.
      </p>
      {isParticipant ? (
        <MinorCorrectionForm
          evaluation={evaluation}
        ...
        <p className="text-xs text-slate-500">
          Only people who handled this report may make corrections.
        </p>
      )}
    </div>
  );
  ...
  return (
    <div className="space-y-2">
      ...
      <p className="text-[10px] text-slate-500">
        Only {MINOR_CORRECTION_KEYS.length} whitelisted fields are editable
        during debrief.
      </p>
    </div>
  );
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-emerald-300 bg-emerald-950/20 border border-emerald-900/40 rounded-lg p-3">
        ✓ Signed by the Reporting Senior and locked for editing. Export it from
        the report header for transmission to PERS-32 or printing.
      </p>
      {canDebrief && (
  ...
  return (
    <div className="border-t border-slate-800 pt-4 space-y-3">
      <h4 className="text-xs font-semibold uppercase tracking-wider text-slate-400">
        Recycle / Review History
      </h4>
      <div className="space-y-3 pl-5 border-l border-slate-800">
        {approvals.map((a, idx) => (
          <div key={a.id || idx} className="relative space-y-1">
            <span
              className={`absolute -left-[21px] top-1.5 h-2 w-2 rounded-full ${a.approval_status === "approved" ? "bg-emerald-500" : a.approval_status === "returned" ? "bg-red-500" : "bg-slate-500"}`}
            />
            <div className="flex justify-between items-center text-xs">
              <span className="font-semibold text-slate-200">
                {a.profiles?.last_name}, {a.profiles?.first_name} (
                {a.profiles?.preferred_role})
              </span>
              <span className="px-2 py-0.5 rounded-full text-[9px] font-bold uppercase border bg-slate-800 text-slate-400 border-slate-700">
                {a.approval_status}
              </span>
            </div>
            {a.reviewer_comments && (
