...
> **Naming (v1.5):** the user-facing module is **"Record Readiness Review"** (nav, page title, consent modal, disclaimer) — the old "Board Confidence Analyzer" name implied E7+ selection boards, but the tool serves all paygrades. Internal identifiers (the `/board-confidence` route, `board_*` tables, `boardConfidence*` modules, `scoreBoardConfidence`) are unchanged for stability.

> **v1.5.1 (review corrections):** the continuity hard gate from the first v1.5 cut was replaced after adversarial review — see item (2). Also fixed under review: the record-extract parser (kept the last/only NEC in space-collapsed text; stopped reading "Master Chief" as a degree; handled all-caps degrees and the cycle-first PFA layout; a cycle listing "BCA PASS PRT FAIL" now scores FAIL, never PASS — never inflate); the LaDR considerations parser (rejects table-of-contents entries and bounds each section so front-matter is not harvested as scored milestones; splits numbered items only at sentence boundaries); a zero-weight tuned config now falls back to the default weights with a warning; the record-extract route rejects oversize bodies on Content-Length and caps extracted text; and the "Extract to record" flow merges into the latest record (no lost concurrent edits) with Save blocked during extraction.
...
1. **Blind-spot gate (primary).** No score while any factor with `weight > 0` has
   `confidence === 0`. Zero confidence is a blind spot, not low coverage: the
   factor's full weight enters the composite as points the Sailor is charged for
   and cannot earn. A weighted average cannot express this — the flawless record
   above cleared the coverage floor at 0.79.
