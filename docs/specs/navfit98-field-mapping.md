...
| 90 | `CommentsPitch` | text(8) | `comment_pitch` (block_values, via `resolveCommentPitch`) | **The column is named Pitch but holds POINT strings, so the units cross:** 10-pitch → `"12 POINT"`, 12-pitch → `"10 POINT"` (10 pitch *is* 12 point — see `COMMENT_PITCH`). Previously mapped `"12"` → `"12 POINT"` while the PDF rendered 10.7278 pt. **One stored value's export changes:** `{"comment_pitch":"12"}` went `"12 POINT"` → `"10 POINT"`. That is the truthful value — a legacy `"12"` draft rendered 10.7278 pt, so `"12 POINT"` never described it either. `{"comment_pitch":"10"}` and `{}` are unchanged at `"10 POINT"`. 12-point string still unobserved — open question | yes | `"10 POINT"` |
...
5. **`CommentsPitch` 12-point string** — `"10 POINT"` confirmed. APEX emits it for **12-pitch** (10 pt) and `"12 POINT"` for 10-pitch (12 pt); the latter remains **unobserved** in a real NAVFIT DB — verify by keying a 10-pitch report into NAVFIT 98A and dumping the row. **Exports are not fully stable across the pitch fix:** a row stored `{"comment_pitch":"12"}` previously exported `"12 POINT"` and now exports `"10 POINT"`. The old string described a 10.7278 pt render, which is not a legal size in either unit, so the new one is the correction — but re-exporting such a row after the fix does produce a different value.
