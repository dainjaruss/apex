...
3.  **Rule (Three or more 2.0s — bar).** ⚠️ *Previously missing from this document and from the code.* "Up to two traits … and still maintain … Promotable" means a **third** `2.0` bars `Promotable` outright, leaving only `Progressing` or `Significant Problems`. Para 6-3 states the same threshold as a plain count when listing what makes an Observed/NOB report un-submittable: "a 1.0 in any trait, a single 2.0 or below in Command or Organizational Climate/Equal Opportunity or Character, **three 2.0 trait grades**, or adverse information in the comments."
...
- **Citation:** BUPERSINST 1610.10H, Encl (2), ch. 1, para 1-2, "EVAL BLOCK 45 / [FITREP/CHIEFEVAL] BLOCK 48", pp. 1-16 to 1-17. Rule 3's parallel wording is Encl (2), **para 6-3**, p. 6-1.
