...
*On screen: wrong password → spotlighted error → correct login → dashboard tour.*
...
*On screen: registration form filled → submit → spotlighted "verification link sent" screen.*
...
*On screen: editor opens → scroll admin blocks → Performance Traits step.*
...
### 5. `scene_2b_live_preview` (~16s · ~37w) · *overlay: Block 41 PDF PiP*
*On screen: narrative step → typing into Block 43 → punch-in on the wrap.*
...
### 6. `scene_2c_validation_rules` (~32s · ~75w) · *overlay: BUPERSINST rules card*
*On screen: trait dropped to 1.0 → Verify Rules → spotlighted violations → grade restored → clean re-verify.*
...
*On screen: left = Sailor clicks Route Forward; right = Rater's dashboard, report appears.*
...
*On screen: Rater types corrective feedback → Recycle → history timeline updates.*
...
### 9. `scene_4a_summary_groups` (~28s · ~65w) · *overlay: quota bar PiP*
*On screen: group expands → members + pooled average → punch-in on quota tracker.*
...
*On screen: Sign button → modal: typed name, drawn signature, credentials, consent → Sign & Certify.*
...
### 11. `scene_5b_audit_lock` (~15s · ~35w) · *overlay: audit PiP*
*On screen: audit tab → spotlighted signature entry.*
...
*On screen: export portal → punch-in on rendered PDF → Download → Finalize → dashboard.*
