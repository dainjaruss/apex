...
const mockInsert = vi.fn().mockReturnValue({
  select: vi.fn().mockReturnValue({
    single: vi
      .fn()
      .mockResolvedValue({ data: { id: "mock-audit-id" }, error: null }),
  }),
});
...
describe("APEX Reviewer & Audit Workflow Unit Tests", () => {
  ...
  it("should write to audit logs via logAction", async () => {
    const res = await logAction("mock-eval-id", "mock-user-id", "TEST_ACTION", {
      info: "details",
    });
  ...
  it("should change status to completed when approved", async () => {
    ...
    const res = await approveEvaluation(
      "mock-eval-id",
      "reviewer-id",
      "Approved!",
    );
  ...
  it("should change status to draft when returned for correction", async () => {
    ...
    const res = await returnForCorrection(
      "mock-eval-id",
      "reviewer-id",
      "Needs more description in block 43.",
    );
