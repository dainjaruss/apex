...
describe('APEX Validation Engine Unit Tests', () => {
  ...
  it('should flag Block 29 overflow caused by the 29A box sharing line 1', () => {
    ...
    expect(result.success).toBe(false)
    expect(result.errors.some(e => e.field === 'primary_duties' && e.block === 29)).toBe(true)
  })
  ...
  it('should accept Block 30 Date Counseled as a date, NOT REQ, or NOT PERF', () => {
    ...
    for (const v of ['2025-01-15', '25JAN15', 'NOT REQ', 'NOT PERF']) {
      const ok = {
        ...mockValidEvaluation,
        block_values: { ...mockValidEvaluation.block_values, date_counseled: v },
      }
      expect(runFullValidation(ok).errors.some(e => e.field === 'date_counseled')).toBe(false)
    }
    const bad = {
      ...mockValidEvaluation,
      block_values: { ...mockValidEvaluation.block_values, date_counseled: 'SOMEDAY' },
    }
    const res = runFullValidation(bad)
    expect(res.errors.some(e => e.field === 'date_counseled' && e.block === 30)).toBe(true)
  })
  ...
  it('should require Block 31 Counselor', () => {
    ...
    expect(res.errors.some(e => e.field === 'counselor' && e.block === 31)).toBe(true)
  })

  it('should cap Block 31 Counselor at 22 characters so it fits the form', () => {
    const tooLong = {
      ...mockValidEvaluation,
      block_values: { ...mockValidEvaluation.block_values, counselor: 'A'.repeat(23) },
    }
    expect(runFullValidation(tooLong).errors.some(e => e.field === 'counselor' && e.block === 31)).toBe(true)

    const exactly22 = {
      ...mockValidEvaluation,
      block_values: { ...mockValidEvaluation.block_values, counselor: 'A'.repeat(22) },
    }
    expect(runFullValidation(exactly22).errors.some(e => e.field === 'counselor')).toBe(false)
  })
  ...
  it('should require at least one Block 41 career recommendation (never blank)', () => {
    ...
    expect(runFullValidation(blank).errors.some(e => e.field === 'career_recommendations' && e.block === 41)).toBe(true)

    // A single entry (e.g. "NA") satisfies the mandatory-minimum.
    const oneEntry = { ...mockValidEvaluation, career_recommendations: ['NA', ''] }
    expect(runFullValidation(oneEntry).errors.some(e => e.field === 'career_recommendations')).toBe(false)
  })

  it('should cap each Block 41 career recommendation at 20 characters', () => {
    const tooLong = { ...mockValidEvaluation, career_recommendations: ['A'.repeat(21), ''] }
    expect(runFullValidation(tooLong).errors.some(e => e.field === 'career_recommendations' && e.block === 41)).toBe(true)

    const exactly20 = { ...mockValidEvaluation, career_recommendations: ['A'.repeat(20), 'LPO'] }
    expect(runFullValidation(exactly20).errors.some(e => e.field === 'career_recommendations')).toBe(false)
  })

  it('should reject more than two Block 41 career recommendations', () => {
    const three = { ...mockValidEvaluation, career_recommendations: ['RECRUITER', 'LPO', 'CCC'] }
    expect(runFullValidation(three).errors.some(e => e.field === 'career_recommendations' && e.block === 41)).toBe(true)
  })
  ...
  it('should warn when a starred Block 21 subcategory is not matched in Block 29', () => {
    ...
    const warning = result.warnings.find((w) => w.field === 'billet_subcategory')
  ...
  it('should not warn when a starred Block 21 subcategory matches Block 29', () => {
    ...
    expect(result.warnings.some((w) => w.field === 'billet_subcategory')).toBe(false)
  })
  ...
  it('should reject a Date Reported (Block 9) that is in the future', () => {
    const future = {
      ...mockValidEvaluation,
      block_values: { ...mockValidEvaluation.block_values, date_reported: '2999-01-01' },
    }
    const res = runFullValidation(future)
    expect(res.errors.some(e => e.field === 'date_reported' && e.block === 9)).toBe(true)
  })

  it('should reject a Date Reported (Block 9) that is not ISO YYYY-MM-DD', () => {
    const legacy = {
      ...mockValidEvaluation,
      block_values: { ...mockValidEvaluation.block_values, date_reported: '24JAN15' },
    }
  ...
  it('should reject an impossible calendar Date Reported (Block 9)', () => {
    const feb30 = {
      ...mockValidEvaluation,
      block_values: { ...mockValidEvaluation.block_values, date_reported: '2024-02-30' },
    }
  ...
  it('should reject a Block 21 billet subcategory that is not a table 1-1 code', () => {
    const offList = {
      ...mockValidEvaluation,
      block_values: { ...mockValidEvaluation.block_values, billet_subcategory: 'BOGUS' },
    }
    const res = runFullValidation(offList)
    expect(res.success).toBe(false)
    expect(res.errors.some(e => e.field === 'billet_subcategory' && e.block === 21)).toBe(true)
  })

  it('should accept a valid SPECIAL Block 21 billet subcategory code', () => {
    const special = {
      ...mockValidEvaluation,
      block_values: { ...mockValidEvaluation.block_values, billet_subcategory: 'SPECIAL05' },
    }
    expect(runFullValidation(special).errors.some(e => e.field === 'billet_subcategory')).toBe(false)
  })
  ...
  it('should reject a Block 29A primary duty abbreviation longer than 14 characters', () => {
    const tooLong = {
      ...mockValidEvaluation,
      block_values: { ...mockValidEvaluation.block_values, primary_duty_abbrev: 'A'.repeat(15) },
    }
    const res = runFullValidation(tooLong)
    expect(res.errors.some(e => e.field === 'primary_duty_abbrev' && e.block === 29)).toBe(true)
  })

  it('should accept a Block 29A abbreviation of 14 characters including spaces', () => {
    const ok = {
      ...mockValidEvaluation,
      block_values: { ...mockValidEvaluation.block_values, primary_duty_abbrev: 'WORK CTR SUPVR' }, // 14 chars
    }
    expect(runFullValidation(ok).errors.some(e => e.field === 'primary_duty_abbrev')).toBe(false)
  })
  ...
  it('should ERROR when a 1.0 mark is present but Block 43 comments are empty', () => {
    ...
    expect(res.errors.some(i => i.field === 'comments' && i.block === 43 && /substantiate/i.test(i.message))).toBe(true)
  })
  ...
  it('should WARN and name the block when a 1.0 mark has comments present', () => {
    const e = {
      ...
      trait_grades: { ...mockValidEvaluation.trait_grades, knowledge: '1.0' },
      comments: 'PO2 DOE STRUGGLED WITH CORE TASKS THIS PERIOD; SPECIFIC EXAMPLES FOLLOW.',
    }
    const w = runFullValidation(e).warnings.find(i => i.field === 'comments' && i.block === 43)
  ...
  it('should not flag two 2.0 marks, but should flag three or more', () => {
    const two = {
      ...
      trait_grades: { ...mockValidEvaluation.trait_grades, knowledge: '2.0', work: '2.0' },
    }
    expect(runFullValidation(two).warnings.some(i => i.field === 'comments' && /substantiate/i.test(i.message))).toBe(false)

    const three = {
      ...mockValidEvaluation,
      promotion_recommendation: 'Promotable',
      trait_grades: { ...mockValidEvaluation.trait_grades, knowledge: '2.0', work: '2.0', accomplishment: '2.0' },
    }
    expect(runFullValidation(three).warnings.some(i => i.field === 'comments' && /three or more 2\.0/i.test(i.message))).toBe(true)
  })
  ...
  it('should flag an ungraded trait on an observed report (Block 33-39 required)', () => {
    const e = {
      ...mockValidEvaluation,
      block_values: { ...mockValidEvaluation.block_values, not_observed: false },
      trait_grades: { ...mockValidEvaluation.trait_grades, leadership: undefined },
    }
    const res = runFullValidation(e)
    expect(res.errors.some(i => i.field === 'trait_grades.leadership' && i.block === 39)).toBe(true)
  })
  ...
  it('should not require trait grades on a wholly Not Observed report', () => {
    ...
    const res = runFullValidation(e)
    expect(res.errors.some(i => i.field?.startsWith('trait_grades.'))).toBe(false)
  })
  ...
  it('should not apply the Block 43 substantiation rule to NOB reports', () => {
    ...
    const res = runFullValidation(e)
    expect(res.errors.some(i => i.field === 'comments' && /substantiate/i.test(i.message))).toBe(false)
    expect(res.warnings.some(i => i.field === 'comments' && /substantiate/i.test(i.message))).toBe(false)
  })
})
