...
test.describe("Eval recycle workflow", () => {
  ...
  test("rater recycles one step back with comments and records approval", async ({
    page,
  }) => {
    await loginAsRole(page, "rater");
    await openEvalTab(page, evalId, "review");
    await assertEvalState(evalId, {
      routing_stage: "rater",
    ...
    await page.reload();
    await expect(page.getByText("Recycle / Review History")).toBeVisible({
      timeout: 10_000,
    });
    await expect(
      page.getByText("Please expand Block 43 substantiation."),
    ).toBeVisible();
    await logout(page);
    ...
    await openEvalTab(page, evalId, "review");
    await expect(
      page.getByRole("button", { name: "Route Forward →" }),
    ).toBeVisible();
  });
});
