...
function adminClient() {
  ...
  if (!url || !key) throw new Error('Supabase env vars missing for E2E DB helper')
...
export async function getEvalState(evalId: string) {
  ...
  const { data, error } = await admin.from('evaluations').select('routing_stage, current_holder_id, signature_locked, status').eq('id', evalId).single()
...
export async function assertEvalState(evalId: string, expected: EvalState) {
  ...
  for (const [key, value] of Object.entries(expected)) {
    ...
    if (state[key as keyof EvalState] !== value) {
      throw new Error(`Eval ${evalId}: expected ${key}=${JSON.stringify(value)}, got ${JSON.stringify(state[key as keyof EvalState])}`)
    }
  }
}
...
export async function resetRoutingEval() {
  ...
  const { error } = await admin.from('evaluations').update({
    ...base,
  ...
  if (error) throw new Error(error.message)
  await admin.from('review_approvals').delete().eq('evaluation_id', ids.evals.routing)
}
...
export async function resetRecycleEval() {
  ...
  if (error) throw new Error(error.message)
  await admin.from('review_approvals').delete().eq('evaluation_id', ids.evals.recycle)
}

export async function countReviewApprovals(evalId: string, status?: string) {
  const admin = adminClient()
  let q = admin.from('review_approvals').select('id', { count: 'exact', head: true }).eq('evaluation_id', evalId)
