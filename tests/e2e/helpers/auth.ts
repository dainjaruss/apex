  ...
  const authError = page
    ...
    .then(async () => {
      const msg = await page
        ...
        .textContent();
      throw new Error(
        `Login failed for ${email}: ${msg?.trim() || "unknown auth error"}`,
      );
    });
...
export async function openEvalTab(
  page: Page,
  evalId: string,
  tab: "details" | "review" | "preview" | "audit" = "review",
) {
  await dismissConsent(page);
...
function routeTargetSelect(page: Page, toUserId?: string) {
  if (toUserId) {
    return page
      .getByRole("combobox")
      .filter({ has: page.locator(`option[value="${toUserId}"]`) });
  }
  return page
    .getByRole("combobox")
    .filter({ has: page.getByText(/Route forward to a/) })
    .last();
}
...
export async function routeForward(page: Page, toUserId?: string) {
  ...
  await page
    .waitForResponse(
      (r) =>
        r.url().includes("/rest/v1/profiles") && r.request().method() === "GET",
      { timeout: 15_000 },
    )
    .catch(() => null);
  ...
  if (toUserId) {
    await expect(
      routeSelect.locator(`option[value="${toUserId}"]`),
...
export async function recycleWithComments(page: Page, comments: string) {
  ...
  await page
    .getByRole("button", { name: "← Recycle to Previous Holder" })
    .click();
  await page.waitForTimeout(1500);
}

export async function expectMemberOnDashboard(
  page: Page,
  memberName: string,
  visible: boolean,
) {
