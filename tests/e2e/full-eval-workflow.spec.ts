...
test.describe("Full BUPERS eval routing workflow", () => {
  ...
  test("routes through custody chain, debriefs, signs, locks, and finalizes", async ({
    page,
  }) => {
    ...
    await assertEvalState(evalId, {
      routing_stage: "senior_rater",
      current_holder_id: ids.users.seniorRater,
    });
    await logout(page);
    ...
    await assertEvalState(evalId, {
      ...
      current_holder_id: ids.users.reportingSenior,
    });
    await logout(page);
