...
describe.skipIf(!hasDocker)("migration 009 — profiles privilege state", () => {
  ...
  // Migration 012 drops the 002 unique constraint by its PostgreSQL-truncated
  // autogenerated name. `drop constraint if exists` on a WRONG name is a silent no-op:
  // the migration still applies cleanly, the new constraint is still added, and the old
  // one keeps rejecting the second group — so a reporting senior with both ACT and INACT
  // E-5s still cannot create it. Applying 012 proves nothing on its own; this asserts the
  // outcome the drop exists for.
  describe("6. migration 012 — Block 5 splits a summary group", () => {
    const sql = (q: string) =>
      docker([
        "exec",
        CONTAINER,
    ...
    const insertGroup = (name: string, dutyStatus: string) => {
      try {
        return sql(
          `insert into public.summary_groups
             (name, reporting_senior_id, period_to, grade_rate, promotion_status,
              command_employment, created_by, duty_status, billet_subcategory)
           select '${name}', p.id, '2026-06-15', 'PO2', 'Regular', 'USS NEVERSAIL',
                  p.id, '${dutyStatus}', 'NA'
    ...
    it("accepts ACT and INACT groups that differ only by Block 5", () => {
      expect(insertGroup("012 ACT", "ACT")).toContain("inserted");
      expect(insertGroup("012 INACT", "INACT")).toContain("inserted");
    });

    it("still rejects a genuine duplicate (NULLS NOT DISTINCT)", () => {
      expect(insertGroup("012 ACT again", "ACT")).toMatch(
