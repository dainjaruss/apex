  ...
  await page.addInitScript(() => {
    ...
    window.addEventListener("DOMContentLoaded", () => {
      ...
      cursor.style.cssText =
        "position:fixed;width:28px;height:28px;border-radius:50%;background:rgba(59,130,246,0.35);border:2.5px solid #3b82f6;pointer-events:none;z-index:999999;transition:transform 0.08s ease, left 0.05s linear, top 0.05s linear;transform:translate(-50%, -50%);box-shadow: 0 0 12px rgba(59,130,246,0.6);";
...
async function warmupLogin(browser: any, email: string, attempt = 1) {
  ...
  try {
    ...
    console.log(
      `   session captured: ${authStates[email].cookies?.length ?? 0} cookie(s)`,
    );
  } catch (err) {
...
async function recordScene(
  ...
  if (opts.asUser && !authStates[opts.asUser]) {
    console.error(
      `!! Skipping ${sceneName}: no warmed session for ${opts.asUser}`,
    );
    return;
  }
  console.log(`\n====================================================`);
  ...
  try {
    ...
    if (opts.asUser && page.url().includes("/login") && attempt === 1) {
      console.error(
        `!! ${sceneName} lost its session (ended on /login) - re-warming ${opts.asUser}`,
      );
    ...
    if (sceneUser) {
      authStates[sceneUser] = await context
        .storageState()
        .catch(() => authStates[sceneUser]);
    }
...
async function recordSplitScene(
  ...
  const [ctxL, ctxR] = await Promise.all([
    makeContext(left.user),
    makeContext(right.user),
  ]);
  ...
  try {
    ...
    authStates[left.user] = await ctxL
      .storageState()
      .catch(() => authStates[left.user]);
    authStates[right.user] = await ctxR
      .storageState()
      .catch(() => authStates[right.user]);
...
function slideHTML(
  ...
  const bullets = items
    .map((t, i) => `<li style="animation-delay:${0.6 + i * 0.35}s">${t}</li>`)
    .join("\n");
...
async function recordSlide(
  ...
  await recordScene(
    ...
    name,
    async (page) => {
      await page.setContent(slideHTML(title, subtitle, items, footer), {
        waitUntil: "load",
      });
...
async function main() {
  ...
  if (existsSync(summaryPath)) {
    ...
    console.warn(
      "Warning: stress-evals-summary.json not found. Using fallback IDs.",
    );
  }

  const sailorEval = evals.find((e) => e.routing_stage === "sailor") ||
    evals[3] || { id: "" };
  ...
  try {
    ...
    const appClips = [
      "scene_1a_login_dashboard",
      "scene_1b_register_email",
      "scene_2a_editor_tour",
      "scene_2b_live_preview",
      "scene_2c_validation_rules",
      "scene_3a_routing_split",
      "scene_3b_recycle",
      "scene_4a_summary_groups",
      "scene_5a_signing",
      "scene_5b_audit_lock",
      "scene_6a_export_download",
      "overlay_1_block41_split",
      "overlay_2_forced_dist_warning",
      "overlay_3_audit_trail_sync",
    ];
    ...
    await recordScene(
      ...
      async (page) => {
        ...
        if (await rank.isVisible().catch(() => false)) {
          ...
          if (tag === "SELECT")
            await rank.selectOption({ index: 1 }).catch(() => {});
    ...
    if (sailorEval.id) {
      await recordScene(
        ...
        async (page) => {
          ...
          const narrativeStep = page
            .locator("text=Narrative & Comments")
            .first();
          ...
          const signaturesStep = page
            .locator("text=Signatures & RS Info")
            .first();
          if (await signaturesStep.isVisible().catch(() => false)) {
            await signaturesStep.click();
            await page.waitForTimeout(1000);
            const promoRec = page
              .locator("text=Promotion Recommendation")
              .first();
      ...
      await recordScene(
        ...
        async (page) => {
          ...
          const verify = async (highlightViolations = false) => {
            ...
            if (await modal.isVisible().catch(() => false)) {
              if (highlightViolations)
                await spotlight(page, '[role="dialog"], .modal', 3000);
              await modal
                .evaluate((el) => el.scrollBy({ top: 250, behavior: "smooth" }))
                .catch(() => {});
            }
            await page.waitForTimeout(2000);
            const closeBtn = page
          ...
          if (broke) {
            ...
            await page.waitForTimeout(600);
            const fourOh = page
              .getByRole("button", { name: "4.0", exact: true })
              .first();
            if (await fourOh.isVisible().catch(() => false))
              await fourOh.click();
            await page.waitForTimeout(1200);
    ...
    if (routeEval.id) {
      await recordSplitScene(
        ...
        async (sailor, rater) => {
          ...
          const routeBtn = sailor
            .locator('button:has-text("Route Forward")')
            .first();
          ...
          const newCard = rater
            .locator(`text=${(routeEval.member_name || "").split(",")[0]}`)
            .first();
        ...
        async (page) => {
          ...
          if (await commentBox.isVisible().catch(() => false)) {
            await humanType(
              ...
              "Please quantify the impact in bullet 2.",
            );
            await page.waitForTimeout(1200);
            const recycleBtn = page
              .locator('button:has-text("Recycle")')
              .first();
    ...
    await recordScene(
      ...
      async (page) => {
        ...
        const expandBtn = page
          .locator('button:has-text("View evaluations in this group")')
          .first();
        if (await expandBtn.isVisible().catch(() => false)) {
          await humanClick(
            page,
            'button:has-text("View evaluations in this group")',
          );
          await page.waitForTimeout(3000);
        }
      ...
      await recordScene(
        ...
        async (page) => {
          ...
          if (await signBtn.isVisible().catch(() => false)) {
            ...
            await page.waitForTimeout(1500);
            const modal = page
              .locator('input[placeholder="DOE, JOHN A"]')
              .first();
            if (await modal.isVisible().catch(() => false)) {
              await humanType(
                page,
                'input[placeholder="DOE, JOHN A"]',
                "ENTERPRISE, C O",
              );
              ...
              if (box) {
                ...
                for (let i = 1; i <= 12; i++) {
                  await page.mouse.move(
                    cx + (i * (box.width - 60)) / 12,
                    cy + Math.sin(i * 1.1) * 18,
                    { steps: 4 },
                  );
                }
                await page.mouse.up();
              ...
              await humanType(
                page,
                'input[placeholder="you@navy.mil"]',
    ...
    if (sailorEval.id) {
      await recordScene(
        ...
        async (page) => {
          await page.goto(
            `${BASE_URL}/evaluations/${sailorEval.id}?tab=preview`,
          );
    ...
    await recordScene(
      browser,
      "overlay_4_validation_rules",
      async (page) => {
        const rules = [
          [
            "Blocks 10–13",
            "Special (13) cannot combine with any other occasion",
          ],
          [
            "Blocks 43/44",
            "Narratives wrap exactly at pitch width — this form's CPL × its line count",
          ],
          [
            "Block 43",
        ...
        const lis = rules
          .map(
            ([blk, txt], i) =>
              `<li style="animation-delay:${0.4 + i * 0.6}s"><b>${blk}</b><span>${txt}</span></li>`,
          )
          .join("");
