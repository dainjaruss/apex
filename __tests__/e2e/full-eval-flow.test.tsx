  ...
  return {
    createBrowserClient: () => ({
      from: (table: string) => {
        return {
          insert: (payload: any[]) => {
            const added = payload.map((item) => ({
              ...item,
              id: `mock-${table}-id-${Math.random()}`,
            ...
            if (table === "review_approvals")
              mockReviewApprovalsTable.push(...added);
          ...
          update: (payload: any) => {
            return {
              eq: (field: string, val: any) => {
                ...
                const updatedItem = mockEvaluationsTable.find(
                  (item) => item.id === val,
                ) || { id: val, ...payload };
                return {
                  select: () => ({
                    single: () =>
                      Promise.resolve({ data: updatedItem, error: null }),
                  }),
                };
              },
            };
...
describe("APEX Full Evaluation Lifecycle E2E Integration Test", () => {
  it("should run the entire evaluation lifecycle successfully", async () => {
    ...
    const correctedReport: Evaluation = {
      ...
      comments:
        "PO2 DOE HAS PERFORMED OUTSTANDING DUTIES THROUGHOUT THIS CYCLE.",
