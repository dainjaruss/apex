...
export function emptyDistribution(): RecDistribution {
  return { 'Significant Problems': 0, Progressing: 0, Promotable: 0, 'Must Promote': 0, 'Early Promote': 0 }
}
...
export function tallyRecommendations(
  recs: Array<string | null | undefined>,
): { distribution: RecDistribution; observedCount: number } {
  const distribution = emptyDistribution()
  for (const r of recs) {
    if (r && (OBSERVED_RECS as readonly string[]).includes(r)) distribution[r as ObservedRec]++
  }
...
export function checkForcedDistribution(
  ...
  const earlyPromoteMax = observedCount > 0 ? Math.ceil(observedCount * 0.2) : 0
  const combinedMax = isE5E6 ? (observedCount > 0 ? Math.ceil(observedCount * 0.6) : 0) : null
  ...
  return { distribution, observedCount, paygrade, isE5E6, earlyPromoteMax, combinedMax, violations, compliant: violations.length === 0 }
}
