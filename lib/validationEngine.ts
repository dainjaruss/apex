...
export function runFullValidation(evalData: Evaluation): ValidationResult {
  ...
  const validationPayload = {
    ...
    career_recommendations: (evalData.career_recommendations || []).filter((r) => (r || '').trim() !== ''),
  ...
  const occasionCount = [bv.periodic, bv.detachment_individual, bv.promotion_frocking, bv.special].filter(Boolean).length
  if (occasionCount === 0) {
    errors.push({
      field: 'occasion',
      block: 10,
    ...
    errors.push({
      field: 'occasion',
      block: 13,
      message: 'Special (Block 13) cannot be combined with another occasion. Do not submit a Special report if another occasion applies (BUPERSINST 1610.10H).',
      severity: 'error',
    })
  }
  ...
  const typeCount = [bv.not_observed, bv.regular_report, bv.concurrent_report].filter(Boolean).length
  if (typeCount === 0) {
    errors.push({
      field: 'type',
      block: 16,
  ...
  if (bv.not_observed) {
    warnings.push({
      field: 'type',
      block: 16,
      message: 'Not Observed (Block 16): trait grades must be left blank and the promotion recommendation must be NOB (BUPERSINST 1610.10H, ch. 6).',
      severity: 'warning',
    })
  }
  ...
  ] as [string, string | undefined][]).forEach(([field, value]) => {
    const spec = FIELD_FIT[field]
    const fit = measureTextFit(value || '', spec.charsPerLine, spec.maxLines, spec.firstLineLead ?? 0)
    if (!fit.fit) {
      errors.push({
        field,
        block: spec.block,
  ...
  if ((STARRED_BILLET_SUBCATEGORIES as readonly string[]).includes(subcategory)) {
    const block29 = `${bv.primary_duty_abbrev || ''} ${bv.primary_duties || ''}`.toUpperCase()
  ...
  const twoCount = (Object.keys(traitBlockMap) as string[]).filter((k) => grades[k] === '2.0').length

  const substReasons: string[] = []
  if (onesBlocks.length) substReasons.push(`a 1.0 mark in ${onesBlocks.join(', ')}`)
  if (twoCount >= 3) substReasons.push(`three or more 2.0 marks (${twoCount} present)`)
  if (grades.eo === '2.0') substReasons.push('a 2.0 in Block 35 (Command/Organizational Climate/EO)')

  const substApplies =
    substReasons.length > 0 && evalData.promotion_recommendation !== 'NOB' && !bv.not_observed
  if (substApplies) {
