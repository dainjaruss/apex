...
import { PDFDocument, PDFFont, PDFPage, rgb, StandardFonts, pushGraphicsState, popGraphicsState, translate } from 'pdf-lib'
...
const REC_COLS = ['NOB', 'Significant Problems', 'Progressing', 'Promotable', 'Must Promote', 'Early Promote']
...
const C = {
  ...
  p1: {
    ...
    name_x: 25, grade_x: 295, desig_x: 360, dodid_x: 460,
    ...
    row69Baseline: 726,
    uic_x: 174, ship_x: 226, promo_x: 421, datereported_x: 500,

    // occasion 10-13 (single row of square centers)
    occasionCy: 707.4,
    periodicCx: 80.6, detachIndCx: 160.6, promoFrockCx: 254.9, specialCx: 333.4,

    // type 16-18
    notObservedCx: 80.6, notObservedCy: 695.1,
    regularCx: 160.6, regularCy: 682.9,
    concurrentCx: 254.9, concurrentCy: 682.9,
    ...
    pfa_x: 365, billet_x: 465,

    // reporting senior 22-27 (cell-lefts measured from blank: 167.8/218.2/269.3/401.0/455.8)
    rsBaseline: 652,
    rsName_x: 26, rsGrade_x: 180, rsDesig_x: 230, rsTitle_x: 280, rsUic_x: 409, rsDodid_x: 463,

    // block 28 narrative (full width; 3 lines)
    b28_x: 21, b28_topBaseline: 630, b28_cpl: FIELD_FIT.command_achievements.charsPerLine, b28_lines: FIELD_FIT.command_achievements.maxLines,
    ...
    b29_firstBaseline: 581, b29_abbrevSize: 12,
    b29b_x: 28, b29b_contX: 25, b29b_cpl: FIELD_FIT.primary_duties.charsPerLine, b29b_lines: FIELD_FIT.primary_duties.maxLines,

    // counseling 30-32 (cell-lefts measured: 196.6/275.0/412.6)
    counselBaseline: 522,
    dateCounseled_x: 200, counselor_x: 279, counselor_width: 130,

    // trait rows 33-37 (grade-box row centers)
    traitCy: { knowledge: 392.8, work: 309.2, eo: 224.3, bearing: 140.1, accomplishment: 56.5 } as Record<string, number>,
  },

  p2: {
    identityBaseline: 749,
    name_x: 22, grade_x: 293, desig_x: 360, dodid_x: 460,
    ...
    b44_x: 22, b44_topBaseline: 245, b44_cpl: FIELD_FIT.qualifications.charsPerLine, b44_lines: FIELD_FIT.qualifications.maxLines,
    ...
    // block 47 retention
    retentionCy: 212.1,
    retentionNotRecCx: 463.7, retentionRecCx: 543.6,
    ...
    rsAddr_x: 390, rsAddr_topBaseline: 182, rsAddr_cpl: FIELD_FIT.reporting_senior_address.charsPerLine, rsAddr_lines: FIELD_FIT.reporting_senior_address.maxLines, rsAddr_width: 178,

    // block 51 member statement (box centers measured: 132.5 / 284.4)
    memberStmtCy: 81,
    intendCx: 132.5, doNotIntendCx: 284.4,
    ...
    date51_x: 215, date51_y: 70, date52_x: 515, date52_y: 70,
  },
}

export async function generateOverlayPdf(evaluation: Evaluation, templateBytes: ArrayBuffer | Uint8Array): Promise<Uint8Array> {
  const pdf = await PDFDocument.load(templateBytes)
  pdf.registerFontkit(fontkit)

  let courier: PDFFont
  try {
    const bytes = fs.readFileSync(path.join(process.cwd(), 'public/fonts/CourierPrime-Regular.ttf'))
  ...
  page1.pushOperators(pushGraphicsState(), translate(OFFSET_P1.dx, OFFSET_P1.dy))
  if (page2 !== page1) page2.pushOperators(pushGraphicsState(), translate(OFFSET_P2.dx, OFFSET_P2.dy))
  ...
  const text = (page: PDFPage, value: string | undefined | null, x: number, y: number, size = 12, font = courier, maxWidth?: number) => {
  ...
  const mark = (page: PDFPage, cx: number, cy: number, size = 11) => {
    ...
    page.drawText('X', { x: cx - w / 2, y: cy - size * 0.36, size, font: markFont, color: BLACK })
  }
  // wrap+draw a monospace narrative; size chosen so `cpl` Courier chars fill `boxWidth`
  const narrative = (page: PDFPage, value: string | undefined, x: number, topBaseline: number, cpl: number, maxLines: number, boxWidth = FORM_RIGHT - FORM_LEFT) => {
    if (!value) return
    const size = Math.max(5, Math.min(12, (boxWidth - 4) / ((cpl + 0.5) * 0.6)))
    const lh = size * 1.18
    const lines = wrapTextToWidth(value, cpl).slice(0, maxLines)
    lines.forEach((ln, i) => page.drawText(ln, { x, y: topBaseline - i * lh, size, font: courier, color: BLACK }))
  }
  ...
  const narrativeWithLead = (page: PDFPage, lead: string | undefined, body: string | undefined, x: number, topBaseline: number, cpl: number, maxLines: number, leadSize: number, leadChars: number, contX = x, boxWidth = FORM_RIGHT - FORM_LEFT) => {
    const leadStr = (lead || '').toUpperCase().trim()
    if (!leadStr && !body) return
    const size = Math.max(5, Math.min(12, (boxWidth - 4) / ((cpl + 0.5) * 0.6)))
    const lh = size * 1.18
    if (leadStr) page.drawText(leadStr, { x, y: topBaseline, size: leadSize, font: courier, color: BLACK })
    if (!body) return
    const padded = ' '.repeat(Math.max(0, leadChars)) + body
    const lines = wrapTextToWidth(padded, cpl).slice(0, maxLines)
    lines.forEach((ln, i) => page.drawText(ln, { x: i === 0 ? x : contX, y: topBaseline - i * lh, size, font: courier, color: BLACK }))
  }
  ...
  narrativeWithLead(page1, bv.primary_duty_abbrev, bv.primary_duties, p1.b29b_x, p1.b29_firstBaseline, p1.b29b_cpl, p1.b29b_lines, p1.b29_abbrevSize, FIELD_FIT.primary_duties.firstLineLead ?? 0, p1.b29b_contX)
  ...
  else if (ret.includes('RECOMMEND')) mark(page2, p2.retentionRecCx, p2.retentionCy)

  // block 48 RS address (wrapped to fit the narrow cell; size auto-fits from cpl + width)
  narrative(page2, up(bv.reporting_senior_address), p2.rsAddr_x, p2.rsAddr_topBaseline, p2.rsAddr_cpl, p2.rsAddr_lines, p2.rsAddr_width)
  ...
  text(page2, evaluation.summary_group_average != null ? evaluation.summary_group_average.toFixed(2) : '', p2.summaryAvg_x, p2.summaryAvg_y)
