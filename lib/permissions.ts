  ...
  if (evaluation) {
    switch (action) {
      ...
      case "submit_for_review":
        ...
        return (
          evaluation.created_by === user.id && evaluation.status === "draft"
        );
...
export function getAvailableActions(
  ...
  return allActions.filter((action) =>
    canPerformAction(user, action, evaluation),
  );
}
...
export function getRoleDescription(role: Role): string {
  const descriptions: Record<Role, string> = {
    Sailor:
      "Create and manage your own evaluation reports. Sign as the evaluated member.",
    Rater:
      "Rate subordinates, sign as Rater, and review submitted evaluations.",
    "Senior Rater":
      "Oversee evaluations as Senior Rater with approval and return authority.",
    "Reporting Senior":
      "Full review chain authority including final sign-off and oversight of all evaluations.",
    Admin:
      "System administrator with unrestricted access to all evaluations, users, and settings.",
  };
