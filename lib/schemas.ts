...
export const registerSchema = z.object({
  ...
  dodId: z
    .string()
    .regex(/^[0-9]{10}$/, 'DoD ID must be exactly 10 digits'),
  ...
  uic: z
    .string()
    .max(5, 'UIC must be 5 characters or fewer')
    .optional(),
  ...
  if (isE1toE5 && ['Rater', 'Senior Rater', 'Reporting Senior'].includes(data.role)) {
    ctx.addIssue({
      path: ['role'],
