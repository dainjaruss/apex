...
export const updateStatus = async (
  ...
  if (error) {
    console.error(
      `updateStatus failed for ID ${id} to ${status}:`,
      error.message,
    );
    throw new Error(error.message);
  }
...
const applyReviewTransition = async (
  id: string,
  reviewerId: string,
  t: ReviewTransition,
): Promise<Evaluation> => {
  ...
  const approval = {
    ...
    ...(t.reviewerComments !== undefined
      ? { reviewer_comments: t.reviewerComments }
      : {}),
  };
  ...
  if (revError) {
    console.error(
      `Failed to insert ${t.approvalStatus} review approval for evaluation ${id}:`,
      revError.message,
    );
  }
...
export const fetchReviewApprovals = async (evaluationId: string) => {
  ...
  if (error) {
    console.error(
      `fetchReviewApprovals failed for evaluation ${evaluationId}:`,
      error.message,
    );
    throw new Error(error.message);
  }
...
export const applyMinorCorrection = (
  evaluationId: string,
