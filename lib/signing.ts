...
import {
  createAdminClient,
  createCredentialVerifierClient,
} from "./supabaseClient";
...
export function parseSignRequest(
  body: any,
): { data: ParsedSign } | SignFailure {
  const { evaluationId, block, email, password, typedName, signatureDataUrl } =
    body || {};
  ...
  if (!key)
    return { error: `Block ${block} is not a signature block.`, status: 400 };
  return {
    data: {
      evaluationId,
      blockNum,
...
export async function verifyCredentials(
  email: string,
  password: string,
): Promise<{ signerId: string } | SignFailure> {
...
export function authorizeSigner(
  ...
  if (evaluation.status === "archived" || evaluation.signature_locked) {
    return {
      error: "This report is finalized and cannot be signed.",
      status: 409,
    };
  }
  return null;
...
export async function applySignature(
  ...
  const lockPatch =
    blockNum === 50 ? { signature_locked: true, routing_stage: "locked" } : {};
